...
class UpdatePassword extends BreezyUpdatePassword
{
        ...
        return $form
            ->schema([
                Forms\Components\TextInput::make('current_password')
                    ->label(BreezyTranslation::get('password_confirm.current_password'))
                    ->required()
                    ->password()
                    ->rule('current_password')
                    ->visible(filament('filament-breezy')->getPasswordUpdateRequiresCurrent()),
                Forms\Components\TextInput::make('new_password')
                    ->label(BreezyTranslation::get('fields.new_password'))
                    ->password()
                    ->rules(filament('filament-breezy')->getPasswordUpdateRules())
                    ->required(),
                Forms\Components\TextInput::make('new_password_confirmation')
                    ->label(BreezyTranslation::get('fields.new_password_confirmation'))
                    ->password()
                    ->same('new_password')
                    ->required(),
            ])
    ...
    public function submit()
    {
        ...
        Notification::make()
            ->success()
            ->title(BreezyTranslation::get('profile.password.notify'))
            ->send();
    }
}
